...
<div class="min-h-screen flex items-center justify-center py-12 px-4 relative overflow-hidden"
    style="background: linear-gradient(rgba(0, 0, 0, 0.8), rgba(0, 0, 0, 0.9)), url('{{ asset('sign.jpg') }}') center center / cover no-repeat;">
    ...
    <div class="absolute inset-0 pointer-events-none">
        ...
        <div class="absolute bottom-1/3 right-1/3 w-1.5 h-1.5 bg-gray-500/20 rounded-full animate-pulse"></div>
    </div>

    <div class="relative z-10 w-full max-w-md">
        <div class="bg-gray-900/90 backdrop-blur-sm rounded-2xl shadow-2xl border border-gray-800 p-8">
            <div class="text-center mb-8">
                <h2 class="text-3xl font-extrabold text-white mb-2 tracking-wider">⚔️ Rejoindre l'Ordre</h2>
                <p class="text-gray-400 italic">Devenez un ninja de l'ombre</p>
            </div>
            ...
            <form action="{{ route('register.submit') }}" method="POST" class="space-y-6">
                ...
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-300 mb-2">👤 Nom de guerre</label>
                    <input 
                        type="text" 
                        id="name" 
                        name="name"
            ...
            <div class="mt-8 text-center">
                <p class="text-gray-400 text-sm">
                    Déjà membre ? 
                    <a href="{{ route('login') }}" class="text-red-400 hover:text-red-300 underline font-semibold transition-colors duration-200">
                        Connectez-vous
                    </a>
                </p>
            </div>
